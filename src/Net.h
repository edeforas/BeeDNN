...
public:
    ...
    void add_dense_layer(int outSize, bool bHasBias=true);
	...
	void add_poolaveraging1D_layer(int outSize);
	void add_poolmax1D_layer(int outSize);
	void add_poolmax2D_layer(int iRowFactor = 2, int iColFactor = 2);
	void add_convolution2D_layer(int iKernelRows, int iKernelCols, int  iOutPlanes);

    void set_input_shape(int iInputRows,int iInputCols=1,int iInputPlanes=1);
	...
	void forward(const MatrixFloat& mIn, MatrixFloat& mOut);
	void classify(const MatrixFloat& mIn, MatrixFloat& mClass);
