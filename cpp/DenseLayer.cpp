...
#include <cstdlib>
#include <cmath>
#include "ActivationSigmoid.h" //todo remove
...
void DenseLayer::forward(const Matrix& mMatIn,Matrix& mMatOut) const
{
    ...
    ActivationSigmoid av; //todo remove
    // apply activation
    for(int i=0;i<mMatOut.size();i++)
    {
        mMatOut(i)=av.forward(mMatOut(i)); //todo keep matrix in layer, do not resize
    }
}
///////////////////////////////////////////////////////////////////////////////
...
Matrix DenseLayer::get_weight_activation_derivation()
{
    ...
    {
        mOut(i)=av.backward(outWeight(i),out(i));
    }
